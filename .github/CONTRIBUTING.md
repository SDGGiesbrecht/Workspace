...
(If the `.Workspace` folder does not exist yet, double‐click `Refresh` first.)
...
To set continuous integration to run a test fork or branch, open `Refresh (manOS).command`...

```shell
open -a Xcode "Refresh (macOS).command"
```
