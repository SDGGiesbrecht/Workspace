...
internal struct ColonSpacing: SyntaxRule {
  ...
  private static let prohibitedPrecedingSpaceMessage = UserFacing<
  ...
  private static let requiredPrecedingSpaceMessage = UserFacing<
    StrictString, InterfaceLocalization
  >({ (localization) in
    switch localization {
    case .englishUnitedKingdom, .englishUnitedStates, .englishCanada:
      return
        "Colons should be preceded by spaces when denoting conformance, inheritance or a ternary condition."
    case .deutschDeutschland:
      return
        "Doppelpunkte sollen vorstehende Leerzeichen haben wenn sie Erfüllung, Erbe oder ein ternären Auswahl bedeuten."
    }
  })
  private static let requiredPrecedingSpaceSuggestion: StrictString = " :"

  private static let requiredFollowingSpaceMessage = UserFacing<
    StrictString, InterfaceLocalization
  >({ (localization) in
    switch localization {
    case .englishUnitedKingdom, .englishUnitedStates, .englishCanada:
  ...
  private static let prohibitedFollowingSpaceMessage = UserFacing<
    StrictString, InterfaceLocalization
  >({ (localization) in
    switch localization {
    case .englishUnitedKingdom, .englishUnitedStates, .englishCanada:
      return
        "Colons should not be followed by spaces when denoting an empty dictionary literal or a function name."
    case .deutschDeutschland:
      return
        "Doppelpunkte sollen keine nachstehende Leerzeichen haben, wenn sie Teil eines leeres selbstdeutendes Wörterbuch oder einen Functionsname sind."
    }
  })
  ...
  internal static func check(
      ...
      token.tokenKind == .colon
    {
        ...
        ternaryExpression.colonMark.indexInParent == token.indexInParent
      {
        ...
        ∨ token.parent is NonEmptyTokenListSyntax
      {
      ...
      if let precedingTrivia = token.firstPrecedingTrivia() {
        ...
        case .backticks, .lineComment, .blockComment, .docLineComment, .docBlockComment,
          ...
          if requiresPrecedingSpace {
            precedingViolation = (
              requiredPrecedingSpaceMessage, requiredPrecedingSpaceSuggestion,
      ...
      if let followingTrivia = token.firstFollowingTrivia() {
        ...
        case .spaces, .tabs, .verticalTabs, .formfeeds, .newlines, .carriageReturns,
          .carriageReturnLineFeeds, .garbageText:
        ...
        case .backticks, .lineComment, .blockComment, .docLineComment, .docBlockComment:
          if requiresFollowingSpace {
            trailingViolation = (
              requiredFollowingSpaceMessage, requiredFollowingSpaceSuggestion,
              token.syntaxRange(in: context)
            )
          }
        }
