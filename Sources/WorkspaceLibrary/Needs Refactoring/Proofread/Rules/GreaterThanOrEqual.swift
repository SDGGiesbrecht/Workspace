...
struct GreaterThanOrEqual : Rule {

    static let name = "Greater Than Or Equal"
    ...
    static func check(file: File, status: inout Bool, output: inout Command.Output) {
        ...
        if let fileType = file.fileType {
            ...
            while let range = file.contents.scalars.firstMatch(for: "\u{3E}=".scalars, in: (index ..< file.contents.endIndex).sameRange(in: file.contents.scalars))?.range.clusters(in: file.contents.clusters) {
