...
struct DivideAndSet : Rule {
    ...
    static func check(file: File, status: inout Bool) {
        ...
        if let fileType = file.fileType {
            ...
            if fileType == .swift {
                message = "Use “÷=” or “divideAccordingToEuclid(by:)” instead. (Import SDGCornerstone.)"
            }

            var index = file.contents.startIndex
            while let range = file.contents.scalars.firstMatch(for: "\u{2F}=".scalars, in: (index ..< file.contents.endIndex).sameRange(in: file.contents.scalars))?.range.clusters(in: file.contents.clusters) {
