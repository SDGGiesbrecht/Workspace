                    ...
                    if ¬line.contains("http")
                        ...
                        case .workspaceConfiguration:
                            if ¬(file.contents.substring(with: file.contents.startIndex ..< range.lowerBound).contains("```shell") ∧ file.contents.substring(with: range.upperBound ..< file.contents.endIndex).contains("```")) /* Shell Script */
                                ...
                                ∧ ¬(file.contents.substring(to: range.lowerBound).contains("[_Begin Other Read‐Me Content_]") ∧ file.contents.substring(from: range.upperBound).contains("[_End_]") ∧ file.contents.substring(to: range.lowerBound).hasSuffix("\n")) /* Other Read‐Me Content */ {
                                throwError()
                            }
