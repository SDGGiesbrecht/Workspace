...
extension Rule {
    ...
    static func line(after match: PatternMatch<String.ScalarView>, in file: TextFile) -> String.ScalarView.SubSequence? {
        let line = lineRange(for: match, in: file)
        guard line.upperBound ≠ file.contents.lines.endIndex else { // [_Exempt from Code Coverage_] Would require additional SwiftLint violations (final empty line), whose order is not deterministic.
            return nil
        }
